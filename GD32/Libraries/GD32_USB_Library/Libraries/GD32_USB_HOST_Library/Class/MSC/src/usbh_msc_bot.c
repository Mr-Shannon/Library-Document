...
/**
  * @brief  Manage the different states of BOT transfer and 
    ...
    if(HCD_IsDeviceConnected(pudev))
    {
        switch (USBH_MSC_BOTXferParam.BOTState)
        {
            ...
            case USBH_MSC_SENT_CBW:
                URB_Status = HCD_GetURB_State(pudev, MSC_Machine.hc_num_out);
                if(URB_Status == URB_DONE)
                {
                    ...
                    if (USBH_MSC_CBWData.field.CBWTransferLength != 0)
                    {
                        ...
                        if (xferDirection == USB_REQ_DIR_D2H)
                        {
            ...
            case USBH_MSC_BOT_DATAIN_STATE:
                URB_Status = HCD_GetURB_State(pudev, MSC_Machine.hc_num_in);
            ...
            case USBH_MSC_DECODE_CSW:
                ...
                if(URB_Status == URB_DONE)
                {
                    ...
                    USBH_MSC_BOTXferParam.BOTXferStatus = USBH_MSC_DecodeCSW(pudev , puhost);
                }
                else if(URB_Status == URB_STALL)
                {
                    error_direction = USBH_MSC_DIR_IN;
...
/**
  * @brief  Decode the CSW received by the device and updates the
  *         same to upper layer.
  * @param  pudev: pointer to usb otg device
  * @param  puhost: pointer to usb host
...
uint8_t USBH_MSC_DecodeCSW(USB_OTG_CORE_HANDLE *pudev, USBH_HOST *puhost)
{
    ...
    if(HCD_IsDeviceConnected(pudev))
    {
        ...
        dataXferCount = HCD_GetXferCount(pudev, MSC_Machine.hc_num_in); 
