/**
  ******************************************************************************
  * @file    ETH/Project/src/HelloGigaDevice.c
  * @author  MCU SD
  * @version V1.0
  * @date    26-Dec-2014
...
static err_t HelloGigaDevice_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err);
static err_t HelloGigaDevice_accept(void *arg, struct tcp_pcb *pcb, err_t err);
static void HelloGigaDevice_conn_err(void *arg, err_t err);
...
{     
    ...
    printf("\n\rTelnet HelloGigaDevice_accept:%d.%d.%d.%d  %s", (u8_t)(IPaddress),(u8_t)(IPaddress >> 8),(u8_t)(IPaddress >> 16),(u8_t)(IPaddress >> 24),GREETING);
    ...
    tcp_err(pcb, HelloGigaDevice_conn_err);
    tcp_recv(pcb, HelloGigaDevice_recv);
    
    /* Send out the first message */  
    tcp_write(pcb, iptxt, strlen(iptxt), 1); 
    sprintf((char*)iptxt, "You telnet computer's IP is: %d.%d.%d.%d\n", iptab[3], iptab[2], iptab[1], iptab[0]);
    printf("%s\r\n",iptxt);
    tcp_write(pcb, GD32F10x_STR, strlen((char *)GD32F10x_STR), 1); 
...
/**
  ...
  * @retval None 
  */
void HelloGigaDevice_init(void)
{
    struct tcp_pcb *pcb;
    ...
    tcp_accept(pcb, HelloGigaDevice_accept);   
}
